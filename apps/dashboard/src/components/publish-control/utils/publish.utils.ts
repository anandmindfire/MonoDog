...
export const calculatePublishStats = (packages: Package[]): PublishStats => {
  return {
    ...
    inProgress: packages.filter(p => p.status === 'building' || p.status === 'testing').length,
...
export const getPublishTypeColor = (type: string): string => {
  switch (type) {
    ...
    case 'prerelease': return 'bg-yellow-100 text-yellow-800';
    default: return 'bg-gray-100 text-gray-800';
  }
};

// Filter packages by name
export const filterPackagesByName = (packages: Package[], selectedPackage: string): Package[] => {
