...
export default function PackageReleaseTable({ packages, selectedPackage, onPackageChange }: PackageReleaseTableProps) {
  return (
    <div className="bg-white rounded-lg shadow border overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-medium text-gray-900">Package Release Status</h3>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {packages.map((pkg) => (
              <tr key={pkg.name} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center">
                    ...
                    <div className="ml-3">
                      ...
                      <div className="text-sm text-gray-500">{pkg.dependencies.length} dependencies</div>
                    </div>
                  </div>
                </td>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getPublishTypeColor(pkg.publishType)}`}>
                    {pkg.publishType}
                  </span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <div className="flex space-x-2">
                    ...
                    <button className="text-purple-600 hover:text-purple-900">Changelog</button>
                  </div>
                </td>
              </tr>
            ))}
