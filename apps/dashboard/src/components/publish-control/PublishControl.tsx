...
export default function PublishControl() {
  ...
  const getPublishStatus = (status: string): 'ready' | 'building' | 'testing' | 'published' | 'failed' => {
    switch (status) {
      case 'healthy': return 'ready';
      case 'warning': return 'testing';
  ...
  const getPublishType = (): 'patch' | 'minor' | 'major' | 'prerelease' => {
    const types: ('patch' | 'minor' | 'major' | 'prerelease')[] = ['patch', 'minor', 'major', 'prerelease'];
