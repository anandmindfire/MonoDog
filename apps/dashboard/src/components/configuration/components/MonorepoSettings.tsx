...
export default function MonorepoSettings({ config, onConfigChange }: SettingsComponentProps) {
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">Choose your monorepo directory structure</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Package Manager
            ...
            <select
              ...
              onChange={(e) => handleMonorepoChange('packageManager', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            >
            ...
            <p className="text-xs text-gray-500 mt-1">Package manager used in your monorepo</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Package Directories
            ...
            <input
              ...
              onChange={(e) => handleMonorepoChange('directories', stringToArray(e.target.value))}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              placeholder="apps, packages, libs, services"
            />
            <p className="text-xs text-gray-500 mt-1">Directories containing packages</p>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">
              Ignore Patterns
            ...
            <input
              ...
              onChange={(e) => handleMonorepoChange('ignorePatterns', stringToArray(e.target.value))}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              placeholder="node_modules, dist, .git"
            />
            <p className="text-xs text-gray-500 mt-1">Patterns to ignore when scanning</p>
          </div>
        </div>
        ...
        <div className="mt-6 p-4 bg-gray-50 border border-gray-200 rounded-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div>
              <span className="font-medium text-gray-700">Ignore Patterns:</span>
              <span className="ml-2 text-gray-600">
        ...
        {config.monorepo?.packageManager && (
          <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <div className="text-xs text-blue-800">
              {config.monorepo.packageManager === 'pnpm' && (
                <p>Make sure you have a pnpm-workspace.yaml file in your root directory.</p>
              )}
              {config.monorepo.packageManager === 'npm' && (
                <p>Configure workspaces in your package.json with the "workspaces" field.</p>
              )}
              {config.monorepo.packageManager === 'yarn' && (
                <p>Ensure you have workspaces configured in your package.json.</p>
              )}
              {config.monorepo.packageManager === 'lerna' && (
                <p>Make sure you have a lerna.json configuration file in your root.</p>
              )}
              {config.monorepo.packageManager === 'nx' && (
                <p>Ensure you have an nx.json configuration file in your workspace root.</p>
              )}
            </div>
          </div>
        )}
