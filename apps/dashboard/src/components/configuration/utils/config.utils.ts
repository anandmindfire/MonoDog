import { DashboardConfig, TabDefinition, ConfigurationTab } from '../types/config.types';
...
export const mergeWithDefaults = (config: Partial<DashboardConfig>): DashboardConfig => {
...
export const validateConfig = (config: DashboardConfig): { isValid: boolean; errors: string[] } => {
  const errors: string[] = [];
  ...
  if (config.monorepo?.directories && config.monorepo.directories.length === 0) {
