...
// Import types
import { PackageDetail as PackageDetailType, PackageDetailTab } from './types/packages.types';
...
const mockPackageDetail: PackageDetailType = {
  ...
  dependencies: [
    ...
    { name: '@types/react', version: '18.0.0', latest: '18.2.0', status: 'outdated' },
    { name: 'tailwindcss', version: '3.3.0', latest: '4.0.0', status: 'major-update' },
  ],
  devDependencies: [
    { name: 'jest', version: '29.0.0', latest: '29.7.0', status: 'outdated' },
    { name: '@testing-library/react', version: '13.0.0', latest: '14.1.0', status: 'major-update' },
    { name: 'storybook', version: '7.0.0', latest: '7.6.0', status: 'outdated' },
  ],
...
export default function PackageDetail() {
  ...
  const [packageData, setPackageData] = useState<PackageDetailType | null>(null);
        ...
        return (
          <div className="py-6">
            <div className="bg-white border rounded-lg p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Package Overview</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="text-sm font-medium text-gray-700 mb-2">Description</h4>
                  <p className="text-gray-600 mb-4">{packageData.description}</p>
                  
                  <h4 className="text-sm font-medium text-gray-700 mb-2">Maintainers</h4>
                  <div className="space-y-1">
                    {packageData.maintainers.map(maintainer => (
                      <div key={maintainer} className="text-sm text-gray-600">{maintainer}</div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{packageData.devDependencies.length}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Scripts:</span>
                      <span className="font-medium">{Object.keys(packageData.scripts).length}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-gray-600">Recent Commits:</span>
                      <span className="font-medium">{packageData.recentCommits.length}</span>
                    </div>
                  </div>
                </div>
              </div>
