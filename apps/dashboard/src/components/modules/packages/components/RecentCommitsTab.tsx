...
export default function RecentCommitsTab({ packageData }: RecentCommitsTabProps) {
  return (
    <div className="py-6">
      ...
      <div className="space-y-4">
        {packageData.recentCommits.map((commit) => (
          <div key={commit.hash} className="bg-white border rounded-lg p-4 hover:shadow-md transition-shadow">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <div className="flex items-center space-x-3 mb-2">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getCommitTypeColor(commit.type)}`}>
                    {commit.type}
                  </span>
                  <span className="text-sm font-mono text-gray-500">{commit.hash}</span>
                </div>
                
                <p className="text-sm font-medium text-gray-900 mb-2">{commit.message}</p>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <span className="ml-2 text-gray-600">
              {packageData.recentCommits.filter(c => c.type === 'feature').length}
            </span>
          </div>
          <div>
          ...
          <div>
            ...
            <span className="ml-2 text-gray-600">
              {packageData.recentCommits.filter(c => c.type === 'breaking').length}
            </span>
          </div>
        </div>
      </div>
