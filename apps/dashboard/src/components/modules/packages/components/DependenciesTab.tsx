...
export default function DependenciesTab({ packageData }: DependenciesTabProps) {
  const renderDependencyTable = (dependencies: PackageDetail['dependencies'], title: string) => (
    <div className="mb-8">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {dependencies.map((dep) => (
              <tr key={dep.name} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className="text-sm font-mono text-gray-700">{dep.version}</span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className="text-sm font-mono text-gray-700">{dep.latest}</span>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getDependencyStatusColor(dep.status)}`}>
                    {dep.status.replace('-', ' ')}
                  </span>
                </td>
    ...
    <div className="py-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
          <div>
            <span className="font-medium text-blue-800">Total Dependencies:</span>
            <span className="ml-2 text-blue-700">{packageData.dependencies.length}</span>
          </div>
          <div>
            <span className="font-medium text-blue-800">Dev Dependencies:</span>
            <span className="ml-2 text-blue-700">{packageData.devDependencies.length}</span>
          </div>
          <div>
            <span className="font-medium text-blue-800">Outdated:</span>
            <span className="ml-2 text-blue-700">
              {[...packageData.dependencies, ...packageData.devDependencies].filter(d => d.status === 'outdated').length}
            </span>
          </div>
          <div>
            <span className="font-medium text-blue-800">Major Updates:</span>
            <span className="ml-2 text-blue-700">
              {[...packageData.dependencies, ...packageData.devDependencies].filter(d => d.status === 'major-update').length}
            </span>
          </div>
        </div>
      </div>
