...
export default function ConfigurationTab({ packageData }: ConfigurationTabProps) {
  return (
    <div className="py-6">
      {/* Scripts */}
      <div className="bg-white border rounded-lg p-6 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Object.entries(packageData.scripts).map(([scriptName, command]) => (
            <div key={scriptName} className="border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-sm font-medium text-gray-900">{scriptName}</h4>
...
{`{
  ...
  "scripts": {${Object.entries(packageData.scripts).map(([key, value]) => `
    "${key}": "${value}"`).join(',')}
  },
  "dependencies": {${packageData.dependencies.map(dep => `
    "${dep.name}": "${dep.version}"`).join(',')}
  },
  "devDependencies": {${packageData.devDependencies.map(dep => `
    "${dep.name}": "${dep.version}"`).join(',')}
  }
}`}
          </pre>
