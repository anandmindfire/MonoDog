...
export default function BuildDetails({ build, onClose, onCancel, onRetry }: BuildDetailsProps) {
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          ...
          <div className="flex items-center space-x-2">
            ...
            {(build.status === 'failed' || build.status === 'cancelled') && onRetry && (
              <button
        ...
        <div className="p-6 overflow-y-auto max-h-[70vh]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            <div className="space-y-4">
              <div>
                <h3 className="text-sm font-medium text-gray-700 mb-2">Build Information</h3>
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{formatDateTime(build.startTime)}</span>
                  </div>
                  {build.endTime && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Ended:</span>
                      <span className="font-medium">{formatDateTime(build.endTime)}</span>
                    </div>
                  )}
                  {build.duration && (
                    <div className="flex justify-between">
                      <span className="text-gray-600">Duration:</span>
                      <span className="font-medium">{formatDuration(build.duration)}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
          ...
          {/* Build Stages */}
          <div>
            <h3 className="text-lg font-medium text-gray-900 mb-4">Build Stages</h3>
            <div className="space-y-4">
              {build.stages.map((stage, index) => (
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center space-x-3">
                      <span className="text-2xl">{getStageIcon(stage.name)}</span>
                      <div>
                  ...
                  {stage.logs && stage.logs.length > 0 && (
                    <div className="mt-3">
                      <h5 className="text-xs font-medium text-gray-700 mb-2">Logs</h5>
