import { Build, BuildStage, Pipeline, BuildStats, CIFilters } from '../types/ci.types';
...
export const filterBuilds = (builds: Build[], filters: CIFilters): Build[] => {
  return builds.filter(build => {
    const matchesPackage = filters.package === 'all' || build.packageName === filters.package;
    const matchesStatus = filters.status === 'all' || build.status === filters.status;
...
export const getBuildProgress = (build: Build): number => {
  ...
  const completedStages = build.stages.filter(stage => 
    stage.status === 'success' || stage.status === 'failed' || stage.status === 'skipped'
  ).length;
  
  const runningStages = build.stages.filter(stage => stage.status === 'running').length;
  ...
  return [...builds].sort((a, b) => {
    ...
    switch (sortBy) {
      ...
      case 'duration':
        ...
        break;
      case 'status':
        const statusOrder = { 'running': 0, 'pending': 1, 'failed': 2, 'cancelled': 3, 'success': 4 };
...
export const getPipelineHealth = (pipeline: Pipeline): 'healthy' | 'warning' | 'critical' => {
