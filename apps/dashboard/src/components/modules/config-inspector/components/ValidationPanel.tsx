...
export default function ValidationPanel({ validation, configName }: ValidationPanelProps) {
    ...
    return (
      <div className="bg-white rounded-lg shadow border p-6">
        <div className="text-center">
          ...
          <h3 className="text-sm font-medium text-gray-900 mb-1">No Validation Results</h3>
          <p className="text-sm text-gray-500">Select a configuration file to see validation results.</p>
        </div>
      </div>
    );
  }
      ...
      <div className="p-4 border-b border-gray-200">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getValidationColor(overallStatus)}`}>
              ...
              <div className="flex items-start space-x-3">
                ...
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-1">
                    <h4 className="text-sm font-medium text-gray-900">{item.field}</h4>
                    <span className={`inline-flex px-2 py-0.5 text-xs font-semibold rounded-full ${getValidationColor(item.status)}`}>
                      {item.status}
                    </span>
                  </div>
