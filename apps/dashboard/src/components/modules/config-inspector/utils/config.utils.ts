import { ConfigFile, ConfigFilters, ValidationResult } from '../types/config.types';
...
export const filterConfigs = (configs: ConfigFile[], filters: ConfigFilters): ConfigFile[] => {
  return configs.filter(config => {
    const matchesSection = filters.section === 'all' || config.id === filters.section;
...
export const validateConfig = (content: string, filename: string): ValidationResult[] => {
...
// Sort configurations
export const sortConfigs = (configs: ConfigFile[], sortBy: 'name' | 'type' | 'size' | 'modified'): ConfigFile[] => {
