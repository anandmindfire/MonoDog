import { HealthMetric, PackageHealth, HealthAlert, HealthTrend } from '../types/health.types';
...
export const getTrendingMetrics = (metrics: HealthMetric[]): { improving: number; declining: number; stable: number } => {
...
export const filterPackagesByHealth = (packages: PackageHealth[], threshold: number = 80): {
  ...
  return packages.reduce((acc, pkg) => {
    ...
    return acc;
  }, { healthy: [] as PackageHealth[], needsAttention: [] as PackageHealth[], critical: [] as PackageHealth[] });
};

// Generate health alerts from package data
export const generateHealthAlerts = (packages: PackageHealth[]): HealthAlert[] => {
...
export const sortPackagesByHealth = (packages: PackageHealth[], ascending: boolean = false): PackageHealth[] => {
  return [...packages].sort((a, b) => {
    return ascending ? a.overallScore - b.overallScore : b.overallScore - a.overallScore;
  });
};
