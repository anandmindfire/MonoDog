...
import { getStatusColor, getMetricIcon, formatTimeAgo } from '../utils/health.utils';

export default function HealthMetrics({ metrics, selectedMetric, onMetricSelect }: HealthMetricsProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-medium text-gray-900">Key Health Metrics</h3>
        <select
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {metrics
          .filter(metric => selectedMetric === 'all' || metric.name === selectedMetric)
          .map((metric) => (
            <div
              key={metric.name}
              className="bg-white p-4 rounded-lg shadow border hover:shadow-md transition-shadow cursor-pointer"
              onClick={() => onMetricSelect(metric.name === selectedMetric ? 'all' : metric.name)}
            >
      ...
      {metrics.filter(metric => selectedMetric === 'all' || metric.name === selectedMetric).length === 0 && (
