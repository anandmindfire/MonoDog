import { PackageNode, CircularDependency, GraphStats } from '../types/dependency.types';
...
export const getDependencyStatusColor = (depId: string, packages: PackageNode[]): string => {
...
export const calculateGraphStats = (packages: PackageNode[]): GraphStats => {
  ...
  const leafPackages = packages.filter(pkg => pkg.dependencies.length === 0).length;
  
  // Find packages with no dependents (root packages)
  const rootPackages = packages.filter(pkg => pkg.dependents.length === 0).length;
  
  // Calculate max depth and average dependencies
  const maxDepth = calculateMaxDepth(packages);
  const avgDependencies = totalPackages > 0 ? totalDependencies / totalPackages : 0;
...
// Detect circular dependencies
export const detectCircularDependencies = (packages: PackageNode[]): CircularDependency[] => {
...
const calculateHierarchicalLayout = (packages: PackageNode[], width: number, height: number): PackageNode[] => {
  const layers: string[][] = [];
  const visited = new Set<string>();
  ...
  packages.filter(pkg => pkg.dependencies.length === 0).forEach(pkg => {
...
const calculateCircularLayout = (packages: PackageNode[], width: number, height: number): PackageNode[] => {
...
const calculateForceLayout = (packages: PackageNode[], width: number, height: number): PackageNode[] => {
