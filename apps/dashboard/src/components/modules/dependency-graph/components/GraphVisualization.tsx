...
import { getStatusColor, getTypeColor, formatPackageName } from '../utils/dependency.utils';
      ...
      <svg className="w-full h-full absolute inset-0" viewBox="0 0 800 600">
        ...
        {packages.map(pkg => 
          pkg.dependencies.map(depId => {
            ...
            const isHighlighted = hoveredPackage === pkg.id || hoveredPackage === depId || 
                                 selectedPackage === pkg.id || selectedPackage === depId;
        ...
        {packages.map(pkg => {
          ...
          const isConnected = selectedPackage && (
            ...
            <g key={pkg.id}>
              <rect
                ...
                stroke={
                  isSelected ? "#3B82F6" : 
                  isConnected ? "#10B981" :
                  isHovered ? "#6B7280" : "#E5E7EB"
                }
                strokeWidth={isSelected ? "3" : isConnected ? "2" : "2"}
                className="cursor-pointer transition-all duration-200"
                style={{
                  filter: isHovered || isSelected || isConnected ? 'drop-shadow(0 4px 6px rgba(0, 0, 0, 0.1))' : 'none'
                }}
              ...
              <circle
                ...
                r="6"
                fill={
                  pkg.status === 'healthy' ? '#10B981' : 
                  pkg.status === 'warning' ? '#F59E0B' : '#EF4444'
                }
