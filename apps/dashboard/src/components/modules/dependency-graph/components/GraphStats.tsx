  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <div className="bg-white p-4 rounded-lg shadow border">
        <div className="flex items-center">
          ...
          <div className="ml-4">
            ...
            <p className="text-2xl font-semibold text-gray-900">{stats.totalDependencies}</p>
          </div>
        </div>
      </div>
        ...
        <div className="flex items-center">
          <div className={`p-2 rounded-lg ${
            ...
            <ExclamationTriangleIcon className={`w-6 h-6 ${
              stats.circularDependencies > 0 ? 'text-red-600' : 'text-green-600'
            }`} />
          </div>
          <div className="ml-4">
      ...
      <div className="bg-white p-4 rounded-lg shadow border">
        <div className="flex items-center">
          ...
          <div className="ml-4">
            ...
            <p className="text-2xl font-semibold text-gray-900">{stats.maxDepth}</p>
          </div>
        </div>
      </div>
      ...
      <div className="md:col-span-4 bg-white p-4 rounded-lg shadow border">
        <h3 className="text-lg font-medium text-gray-900 mb-4">Dependency Analysis</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{stats.leafPackages}</div>
            <div className="text-sm text-gray-600">Leaf Packages</div>
            <div className="text-xs text-gray-500">No dependencies</div>
          </div>
          
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">{stats.rootPackages}</div>
            <div className="text-sm text-gray-600">Root Packages</div>
            <div className="text-xs text-gray-500">No dependents</div>
          </div>
          
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-600">{stats.avgDependencies}</div>
          ...
          <div className="text-center">
            ...
            <div className="text-xs text-gray-500">
              {Math.round((packages.filter(p => p.status === 'healthy').length / packages.length) * 100)}% of total
            </div>
          </div>
        </div>
      </div>
