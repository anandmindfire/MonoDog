...
import { getStatusColor, getTypeColor, getDependencyStatusColor } from '../utils/dependency.utils';

export default function DependencyDetails({ package: pkg, packages, onClose }: DependencyDetailsProps) {
  if (!pkg) return null;
      ...
      <div className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-gray-500">({pkg.dependencies.length})</span>
          </div>
          {pkg.dependencies.length > 0 ? (
            <div className="space-y-1 max-h-32 overflow-y-auto">
              {pkg.dependencies.map(depId => {
                ...
                return dep ? (
                  <div key={depId} className="flex items-center justify-between text-sm py-1">
                    <span className="text-gray-600 truncate">{dep.name}</span>
                    <div className="flex items-center space-x-1">
                      <span className="text-xs text-gray-400">{dep.version}</span>
                      <span className={`${getDependencyStatusColor(depId, packages)}`}>
                        {dep.status === 'healthy' ? '✓' : dep.status === 'warning' ? '⚠' : '✗'}
                      </span>
                    </div>
                  </div>
                ) : (
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-xs text-gray-500">({pkg.dependents.length})</span>
          </div>
          ...
          <div className="space-y-1 text-xs text-gray-600">
            ...
            <div className="flex justify-between">
              ...
              <span className={`font-medium ${
                pkg.dependents.length > 5 ? 'text-red-600' :
                pkg.dependents.length > 2 ? 'text-yellow-600' : 'text-green-600'
              }`}>
                {pkg.dependents.length > 5 ? 'High' :
                 pkg.dependents.length > 2 ? 'Medium' : 'Low'}
              </span>
            </div>
          </div>
        </div>
