...
export const calculatePackageStats = (packages: Package[]): PackageStats => {
  return {
    ...
    custom: packages.filter(p => !['app', 'lib', 'tool'].includes(p.type)).length,
    totalDependencies: packages.reduce((sum, pkg) => 
      sum + Object.keys(pkg.dependencies).length + Object.keys(pkg.devDependencies).length, 0
    )
  };
};
...
export const getStatusColor = (type: string): string => {
  switch (type) {
    ...
    case 'lib': return 'bg-green-100 text-green-800';
