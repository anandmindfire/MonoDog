...
export default function PackageTable({ packages, getTypeIcon, getStatusColor }: PackageTableProps) {
  return (
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {packages.map((pkg) => (
            <tr key={pkg.name} className="hover:bg-gray-50">
              ...
              <td className="px-6 py-4 whitespace-nowrap">
                <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${getStatusColor(pkg.type)}`}>
                  {pkg.type}
                </span>
              </td>
